  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          image = "${data.aws_caller_identity.current.account_id}.dkr.ecr.${var.primary_region}.localhost.localstack.cloud:4566/${var.web_api_image.name}:${var.web_api_image.version}"
          name  = local.web_api_name
